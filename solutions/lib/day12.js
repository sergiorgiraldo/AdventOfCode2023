...
function getBrokenSpringsUnfolded(lines) {
	...
	for (let i = 0; i < field.length; i++) {
		...
		const possibility = checkConditionsPerLine(
			field[i].springs,
			conditions
		);
...
function generateCombinations(springs) {
	...
	function recurse(springs) {
		const questionMarkPosition = springs.indexOf("?");

		if (questionMarkPosition >= 0) {
			const dotString =
				springs.substring(0, questionMarkPosition) +
				"." +
...
const checkConditionsPerLine = helpers.memoize(function (line, conditions) {
	...
	if (
		line.length <
		conditions.reduce((acc, curr) => acc + curr, 0) + conditions.length - 1
	) {
		return 0;
	}
	...
	if (line[0] === "#") {
		...
		return checkConditionsPerLine(
			line.slice(condition + 1),
			remainingConditions
		);
	}
